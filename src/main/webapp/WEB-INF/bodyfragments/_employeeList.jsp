...
<c:url var="addUrl" value="/ctl/employee" />

<c:url var="addSearch" value="/ctl/employee/search" />

<c:url var="editUrl" value="/ctl/employee?id=" />
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<div class="container-fluid">
			...
			<a href="${pageContext.request.contextPath}/ctl/employee"
						...
						<div class="card">
							...
							<div class="card-body">
								<table id="example2" class="table table-bordered table-hover">
									...
									<tbody>
										<c:forEach items="${list}" var="emp" varStatus="employee">
											<tr>
												<td><input type="checkbox" class="case" name="ids"
													value="${emp.id}"></td>
												<td>${employee.index+1}</td>
												<td><c:out value="${emp.firstName} ${emp.lastName}" />
												</td>
												<td><c:out value="${emp.email}" /></td>
												<td><c:out value="${emp.mobileNo}" /></td>
												<td><c:out value="${emp.address}" /></td>
												<td><a href="${editUrl} ${emp.id}"
